 ...
 */
	...
	//
	if ($DEBUG && droit('DEV') && !empty($_POST['url_data'])) {
	...
	$nbRows = (empty($_POST['nbRows'])) ? empty($settings_nbRows) ? 500 : (int)$settings_nbRows : (int)$_POST['nbRows'];
	//
	...
	if (!empty($groupBy) && !empty($rs_dist) && !empty($groupBy_field) ):
		...
		switch($groupBy_mode):
			...
			case 'field':
				...
				if(strpos($groupBy_field,'date')!==false){
					$rs_dist = array_map(function($value){return  substr($value, 0, 7);},$rs_dist);
					$groupBy_mode = 'date';
				}else if(strpos($groupBy_field,'email')!==false){
					$rs_dist = array_map(function($value){return  end(explode('@',$value));},$rs_dist);
					$groupBy_mode = 'email';
				}else if(is_string(reset($rs_dist))){
					$rs_dist = array_map(function($value){return  substr($value, 0, 2);},$rs_dist);
					$groupBy_mode = 'string';
				}else{
		...
		foreach ($rs_dist as $key_dist => $arr_dist):
			...
			$data_main[] = ['groupBy_key' => $groupBy_key,'html' => $z, 'md5' => md5($z), 'order' => $i, 'table' => $groupBy, 'table_value' => $table_value, 'vars' => $vars_rfk, 'groupBy' => 1, 'maxcount' => $dist_max_count, 'count' => $dist_count];
		...
		if ($rs_noGroupBy->count() != 0):
			...
			$groupBy_key = 'groupby_'. $groupBy.'_idx_'.$i;
			$data_main[] = ['html' => $z, 'vars' => $vars_rfk,'groupBy_key' => $groupBy_key, 'groupBy' => 1];
			$strm[]      = ['html' => $z, 'table' => $groupBy,'groupBy_key' => $groupBy_key, 'table_value' => 'no_group', 'vars' => $vars_rfk, 'groupBy' => 1];
	...
	if (empty($groupBy)):
		...
		while ($arr = $rs->getNext()) {
			...
			$data_main[] = array_merge($data_row, ['order' => $i, 'chunk' => $count_frag_test, 'chunks' => ceil($count / $strem_chunk_size), 'maxcount' => $maxcount, 'count' => $count]);
			...
			if (!empty($_POST['stream_to'])):
				...
				if (($count_frag % $strem_chunk_size) == 0 || !$rs->hasNext()) {
					$out_model = ['data_main' => $data_main,
					              ...
					              'table'     => $table];
					//
					$strm_vars = ['stream_to' => $_POST['stream_to'], 'data_size' => sizeof($data_main),
					              'table'     => $table,
					              'data'      => $out_model];
	...
	if (!empty($_POST['csv_export'])):
		...
		foreach ($data_main as $key => $value) {
			array_walk($value['html'], 'strip_tags');
			$csv_main[] = $value['html'];
		}
		//
		...
		//
		skelMdl::send_cmd('act_gui', ['title' => 'Télécharger le csv', 'mdl' => 'app/app_prod/app_prod_csv', 'vars' => 'uniqid=' . $uniqid], session_id());
	...
	function dotr($table, $arr) {
		...
		foreach ($arrFields_all as $key_f => $value_f):
			...
			$out[$field_name]        = $APP->cast_field($arr_cast);
			if ($codeAppscheme_field_type == 'bool') {
				$set_value        = empty($field_value) ? 1 : 0;
