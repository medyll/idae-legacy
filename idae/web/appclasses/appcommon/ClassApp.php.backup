<?php
	...
	class App extends \MongoClient {
		 ...
		 * @param $table
		 *
		 * @return array base_fk collection_fk table_fk idtable_fk
		 */
		function get_grille_fk($table = '', $vars = []) {
			...
			foreach ($grille_fk as $arr_fk):
				...
				foreach ($arr_has as $key => $value):
					$Value = ucfirst($value);
					//vardump_async($db_fk,true);
					if ($db_fk['is' . $Value . 'Scheme']) {
						$continue = true;
					}
				endforeach;
		...
		function get_field_group_list($codeGroupe = '', $qy_has_field = [], $excludedCode = []) {
			...
			while ($arrg = $rsG->getNext()) {
				...
				$rstmp_ar              = $this->appscheme_has_field->find(['idappscheme' => $this->idappscheme, 'idappscheme_field' => ['$in' => $arr_field_tmp]])->sort(['ordreAppscheme_has_field' => 1]);

				while ($arrf = $rstmp_ar->getNext()) {
					$tmp_ar = $this->appscheme_field->findOne(['idappscheme_field' => (int)$arrf['idappscheme_field']]);
					$out2[] = array_merge($tmp_ar, $arrf);//$tmp_ar;
		...
		function get_reverse_grille_fk($table, $table_value = '', $add = []) {
			...
			foreach ($rs as $arr):
				if (in_array($arr['codeAppscheme'], ["agent_recherche", 'agent_tuile', 'agent_liste'])) continue;
				if (empty($table_value)) {
				...
				$index = $rs_fk->count();
				//
				$out[$arr['codeAppscheme']] = $arr + ['count' => $index, 'table' => $arr['codeAppscheme'], 'scope' => $arr['codeAppscheme_type'], 'icon' => $arr['icon'], 'table_value' => $table_value/*,  'rs_fk'=>$rs_fk*/];
			endforeach;
...
//
		...
		function get_settings($idagent, $key, $table = '') {
			...
			$arr         = $this->plug('sitebase_pref', 'agent_pref')->findOne(['idagent' => (int)$idagent, 'codeAgent_pref' => $key . $width_table]);
		...
		function set_settings($idagent, $vars) {
			foreach ($vars as $key => $val) {
				...
				$arr = $this->plug('sitebase_pref', 'agent_pref')->findOne(['idagent' => (int)$idagent, 'codeAgent_pref' => $key]);
		...
		function del_settings($idagent, $key, $table = '') {
			$width_table = empty($table) ? '' : '_' . $table;
			$del         = $this->plug('sitebase_pref', 'agent_pref')->remove(['idagent' => (int)$idagent, 'codeAgent_pref' => $key . $width_table]);
		...
		function is_confident_table($table) {
			$conf = ['agent_note', 'todo', 'tache'];

			return in_array($table, $conf);
		}

		function get_array_field_bool() {
			$arr = $arrFieldsBool = ['estTop' => ['star', 'star-o textgris'], 'estActif' => ['check', 'square-o textgris'], 'estVisible' => ['eye-slash', 'eye textgris']];
		...
		public function has_field_fk($table) {
			$arr_test = array_search($table, array_column($this->get_grille_fk($this->table), 'table_fk'));
		...
		function distinct_rs($vars_dist) {
			...
			$sort_field    = empty($vars_dist['sort_field']) ? ['nom', 1] : $vars_dist['sort_field'];
		...
		function has_field($field) {
			if (is_array($field)) {
				foreach ($field as $key => $value) {
					$arr_test = $this->appscheme_has_field->findOne(['idappscheme' => (int)$this->idappscheme, 'codeAppscheme_field' => $value]);
					if (!empty($arr_test['idappscheme'])) return true;
				}

				return false;
			}
			$arr_test = $this->appscheme_has_field->findOne(['idappscheme' => (int)$this->idappscheme, 'codeAppscheme_field' => $field]);
		...
		function get_fk_tables($table = null) {
			...
			usort($grille_fk, function ($a, $b) { return $a['ordreTable'] > $b['ordreTable']; });
		...
		function get_display_fields($table = '') {
			...
			foreach ($DEFAULT_FIELDS as $key_df => $value_df) {
				...
				if (!empty($APP_TABLE['has' . ucfirst($Key) . 'Scheme'])):
					$default_model[$key_df] = ['field_name' => $key_df . $Table, 'field_name_raw' => $key_df, 'title' => idioma($Name)];
					if ($key_df == 'adresse'):
		...
		function get_basic_fields_nude($table = '') {
			$ix  = "";
			$out = ['nom' . $ix, 'petitNom' . $ix, 'code' . $ix, 'atout' . $ix, 'description' . $ix, 'url' . $ix];
			if ($table == 'feed_header' || $table == 'promo_zone_item') {
				array_push($out, 'url' . $ix);
			}

			return $out;
		}

		function get_basic_fields_query($table = '') {
			$ix = ucfirst($table);
			$fl = $this->get_basic_fields($table);
			foreach ($fl as $field):
				$out[$field] = 1;
			endforeach;
		...
		function get_basic_fields($table = '') {
			$ix  = ucfirst($table);
			$out = ['nom' . $ix, 'petitNom' . $ix, 'code' . $ix, 'email' . $ix, 'atout' . $ix, 'description' . $ix, 'url' . $ix];
		...
		function get_bool_fields($table = '') {
			$ix  = ucfirst($table);
			$arr = $arrFieldsBool = ['estTop' => 'estTop' . $ix, 'estActif' => 'estActif' . $ix, 'estVisible' => 'estVisible' . $ix];

			return $arr;
		}

		function get_meta_fields($table = '') {
			$ix = ucfirst($table);

			return $arrFieldsMeta = ['siteTitle' . $ix, 'siteDesc' . $ix, 'siteInfo' . $ix, 'metaFiche' . $ix, 'metaListe' . $ix, 'metaInfo' . $ix, 'webNom' . $ix, 'webPlurielNom' . $ix];
		}

		function get_sort_fields($table = '') {
			$ix = ucfirst($table);
		...
		function get_http_mdl($mdl, $vars = [], $value = '', $attributes = '') {
			// http_post_data()
			$fields = ['name' => 'mike', 'pass' => 'se_ret'];
			$files  = [['name' => 'uimg', 'type' => 'image/jpeg', 'file' => './profile.jpg',]];
		...
		function consolidate_app_scheme($table) {
			...
			$arr_main   = $APP_SCH_HAS_TABLE->distinct_all('idappscheme_field', ['idappscheme' => $idappscheme, 'idappscheme_link' => $idappscheme]);
			...
			if (sizeof($arr_main) != 0 && sizeof($arr_main_2) != 0) {
				...
				if (sizeof($diff) != 0) {
					$APP_SCH_HAS_TABLE->remove(['idappscheme' => $idappscheme, 'idappscheme_link' => $idappscheme, 'idappscheme_field' => ['$in' => $diff]]);
				}
			}
			...
			if (!empty($ARR_GR['idagent_groupe'])) {
				$APP_DROIT->create_update(['idagent_groupe' => (int)$ARR_GR['idagent_groupe'], 'idappscheme' => $idappscheme], ['C' => true, 'R' => true, 'U' => true, 'D' => true, 'L' => true]);
			}
			...
			$arrSF  = $APP_SCH_HAS->findOne(['idappscheme' => $idappscheme, 'idappscheme_field' => $IDFIELD_NOM]);

			if (empty($arrSF['idappscheme_field']) && $testSF->count() == 0) {
				// echo "<br>champ nom par defaut vide";
				$ins                            = ['codeAppscheme_has_field' => 'nom' . ucfirst($table), 'codeAppscheme_field' => 'nom'];
				...
				$idappscheme_has_field = $APP_SCH_HAS->create_update(['idappscheme' => $idappscheme, 'idappscheme_field' => (int)$IDFIELD_NOM], $ins);
				$APP_SCH_FIELD->consolidate_scheme($idappscheme_has_field);
			}
			$testSF = $APP_SCH_HAS_TABLE->find(['idappscheme' => $idappscheme]);
			while ($ARR_HAST = $testSF->getNext()) {
				$ARR_SCH_FIELD = $APP_SCH->findOne(['idappscheme' => (int)$ARR_HAST['idappscheme_link']]);
				$ARR_FIELD_NOM = $APP_SCH_FIELD->findOne(['idappscheme_field' => (int)$ARR_HAST['idappscheme_field']]);
				$DA_TABLE_CODE = $ARR_SCH_FIELD['codeAppscheme'];
				$DA_TABLE_NAME = $ARR_SCH_FIELD['nomAppscheme'];

				$ins['iconAppscheme_has_table_field']     = $ARR_FIELD_NOM['iconAppscheme_field'];
				$ins['petitNomAppscheme_has_table_field'] = $ARR_FIELD_NOM['petitNomAppscheme_field'] . ' ' . $DA_TABLE_NAME;
				$ins['nomAppscheme_has_table_field']      = $ARR_FIELD_NOM['nomAppscheme_field'] . ' ' . $DA_TABLE_NAME;
			...
			if (!empty($force) || empty($arrSF['codeAppscheme_field']) && $testSF->count() == 0) {

				$ins                                  = ['idappscheme' => $idappscheme, 'idappscheme_link' => $idappscheme, 'idappscheme_field' => $IDFIELD_NOM];
				...
				$idappscheme_has_table_field = $APP_SCH_HAS_TABLE->create_update(['idappscheme' => $idappscheme, 'idappscheme_link' => $idappscheme], $ins);
		...
		function consolidate_scheme($table_value = '') { // nom et codetype, grille_fk
			...
			while ($arr = $rs->getNext()):
				...
				if (!empty($arr['dateDebut' . $Name_table])):
					if (($arr['timeDebut' . $Name_table]) != strtotime($arr['dateDebut' . $Name_table])) $arr_new['timeDebut' . $Name_table] = strtotime($arr['dateDebut' . $Name_table]);
				endif;
				//
				if (!empty($arr['dateFin' . $Name_table])):
					if (($arr['timeFin' . $Name_table]) != strtotime($arr['dateFin' . $Name_table])) $arr_new['timeFin' . $Name_table] = strtotime($arr['dateFin' . $Name_table]);
				endif;
				...
				foreach ($GRILLE_FK as $field):
					...
					$rs_fields = $this->appscheme_has_table_field->find(['idappscheme' => $this->idappscheme, 'idappscheme_link' => (int)$arr_lnk['idappscheme']]);

					// echo $rs_fields->count();
					$arrq = $this->plug($field['base_fk'], $field['table_fk'])->findOne([$id_fk => (int)$arr[$id_fk]]);
					if (empty($arrq)) continue;
					$arr_new[$id_fk] = (int)$arr[$id_fk]; // x86
					foreach ($rs_fields as $arr_fields) {
						if ($arr[$arr_fields['ordreAppscheme_field'] . ucfirst($field['table_fk'])] != $arrq[$arr_fields['ordreAppscheme_field'] . ucfirst($field['table_fk'])]) {
							$arr_new[$arr_fields['ordreAppscheme_field'] . ucfirst($field['table_fk'])] = $arrq[$arr_fields['ordreAppscheme_field'] . ucfirst($field['table_fk'])];
						}
						if ($arr[$arr_fields['codeAppscheme_field'] . ucfirst($field['table_fk'])] != $arrq[$arr_fields['codeAppscheme_field'] . ucfirst($field['table_fk'])]) {
				...
				if ($this->has_field('code')) {
					if (empty($arr['code' . $Name_table]) && !empty($arr['nom' . $Name_table])) {
				...
				if ($name_table == 'newsletter'):
					$new_value = Act::lienNewsletter((int)$arr['idnewsletter']);
					if ($arr['url' . $Name_table] != $new_value) {
						$arr_new['url' . $Name_table] = $new_value;
					};
				endif;
				if ($name_table == 'produit_tarif_gamme'):
					// dans produit=> trnasport
					$APP_TMP                      = new App('produit');
					$APP_TMP_2                    = new App('produit_tarif');
					$arr_test                     = $APP_TMP->findOne(['idproduit' => (int)$arr['idproduit']]);
					$arr_test_2                   = $APP_TMP_2->findOne(['idproduit_tarif' => (int)$arr['idproduit_tarif']]);
					$new_value                    = $arr_test['codeFournisseur'] . ' ' . $arr_test['codeTransport'] . ' ' . $arr['codeTransport_gamme'] . ' ' . date_fr($arr_test_2['dateProduit_tarif']);
					$arr_new['nom' . $Name_table] = $new_value;
					/*$new_value =  Act::lienNewsletter((int)$arr['idnewsletter']);
					if ($arr['nom' . $Name_table] != $new_value) {
						$arr_new['nom' . $Name_table] = $new_value;
				...
				if ($name_table == 'produit_tarif'):
					// dans produit=> trnasport
					$APP_TMP                      = new App('produit');
					$APP_TMP_2                    = new App('produit_tarif');
					$arr_test                     = $APP_TMP->findOne(['idproduit' => (int)$arr['idproduit']]);
					$arr_test_2                   = $APP_TMP_2->findOne(['idproduit_tarif' => (int)$arr['idproduit_tarif']]);
					$new_value                    = date_fr($arr_test_2['dateProduit_tarif']) . ' ' . $arr_test['codeFournisseur'] . ' ' . $arr_test['codeTransport'] . ' ' . $arr['codeTransport_gamme'];
					$arr_new['nom' . $Name_table] = $new_value;
					...
					if ($test_j->count() < 7 || $test_j_s->count() == 0) {
						...
						while ($ARR_JOURS = $rs_j->getNext()) {
							...
							if ($test_j_s->count() < 1) {
								...
								$h_d                 = rand(0, 1);
								$h_f                 = rand(14, 15);
								$idshop_jours_shift  = $APP_SH_J_SHIFT->create_update(['nomShop_jours_shift' => $nomShop_jours_shift . ' AM ' . $nomShop, 'idshop_jours' => $idshop_jours, 'idshop' => $table_value, 'heureDebutShop_jours_shift' => '00:01:00'], ['heureFinShop_jours_shift' => $h_f . ':59:59', 'actifShop_jours_shift' => 1, 'ordreShop_jours_shift' => (int)$ARR_JOURS['ordreJours']]);
								$APP_SH_J_SHIFT->consolidate_scheme($idshop_jours_shift);
								$h_d                = rand(16, 17);
								$h_f                = rand(22, 23);
								$idshop_jours_shift = $APP_SH_J_SHIFT->create_update(['nomShop_jours_shift' => $nomShop_jours_shift . ' PM ' . $nomShop, 'idshop_jours' => $idshop_jours, 'idshop' => $table_value, 'heureDebutShop_jours_shift' => $h_d . ':00:00'], ['heureFinShop_jours_shift' => $h_f . ':59:59', 'actifShop_jours_shift' => 1, 'ordreShop_jours_shift' => (int)$ARR_JOURS['ordreJours']]);
								$APP_SH_J_SHIFT->consolidate_scheme($idshop_jours_shift);
							}
						}
					}
				...
				if (empty(trim($arr['nom' . $Name_table])) && empty($arr_new['nom' . $Name_table])):
					...
					foreach ($REVERSE_GRILLE_FK as $field):
						...
						//
						$arr_new['nom' . $Name_table] .= $dsp_name . ' ';
		...
		function init_scheme($base, $table, $options = [], $force = false) {
			...
			} elseif ($force == true && !empty($test['idappscheme'])) {
			...
			if (!empty($options['fields'])) {
				foreach ($options['fields'] as $key => $value) {
					...
					if (!empty($ARRF['idappscheme_field'])) $arr_field[] = (int)$ARRF['idappscheme_field'];
					else {
			...
			if (!empty($options['grilleFK'])) {
				...
				foreach ($options['grilleFK'] as $key => $value) {
					$ARRFK [(int)$key] = ['table' => $value, 'ordreTable' => (int)$key, 'uid' => uniqid()];
				}
				$ins['grilleFK'] = $ARRFK;
			...
			if (!empty($arr_field)) {
				...
				foreach ($arr_field as $key => $value) {
					$id      = (int)$value;
					$idschas = $APP_SCH_HAS->create_update(['idappscheme' => $ins['idappscheme'], 'idappscheme_field' => $id]);
		...
		function cast_field($vars = [], $nude = false) { // json_data_table !
			...
			switch ($vars['codeAppscheme_field_type']):
				case 'bool':
					...
					$value     = "<i data-set_value='$set_value' class = 'fa fa-$icon  $css'  ></i >$text";
					break;
				case 'valeur':
					$value = $value;
					break;
		...
		function get_field_list_raw($in = []) {
			...
			$a = [];
			while ($arrg = $rsG->getNext()) :
				$test                           = $this->appscheme_field->findOne($DIST + ['idappscheme_field' => (int)$arrg['idappscheme_field']]);
				$a['nomAppscheme_field']        = $test['nomAppscheme_field'];
				$a['nomAppscheme_field_type']   = $test['nomAppscheme_field_type'];
				$a['codeAppscheme_field_type']  = $test['codeAppscheme_field_type'];
				$a['nomAppscheme_field_group']  = $test['nomAppscheme_field_group'];
