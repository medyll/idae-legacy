<?php
	...
	$idagent = (int)$_POST['vars']['idagent'];

	$arrAgent = $APP_AGENT->findOne(array('idagent'=>(int)$idagent));
	...
	foreach($list as $num=>$msg):
		if(!empty($msg['message_id'])):
			...
			$forAll['to'] = $out['to'];
			$forAll['nomEmail'] = $out['nomEmail'];
			//
			skelMdl::send_cmd('act_notify',['msg'=>$out['nomEmail']],session_id());
			//
	...
	endforeach;
	skelMdl::send_cmd('act_notify',['msg'=>'red '],session_id());
	skelMdl::reloadModule('app/app_mail/app_mail_liste',$idagent) ;
?>