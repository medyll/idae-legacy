...
?>
<div style="width:950px;" class="ededed">
	<div class="blanc">
		<div style="overflow:hidden;" id=" ">
			<form action="mdl/app/app_skel/actions.php" onSubmit="ajaxFormValidation(this);return false" auto_close="auto_close">
				<input type="hidden" value="updateForm_modele_perso" name="F_action">
				<input type="hidden" value="close" name="reloadModule[app/app_skel/skelbuilder_liste]">
...
//
								...
								?>
								<tr>
									<td><?= ucfirst(empty($arrf['nomAppscheme_has_field']) ? $arrf['codeAppscheme_has_field'] : $arrf['nomAppscheme_has_field']) ?></td>
									<td><?= chkSch($act_vars, $arr_test_has['idappscheme'], 'vars_has_table_field'); ?></td>
								</tr>
							<? } ?>
						</table>
					</div>
							...
							<?
								...
								foreach ($ARR_HAS as $TYPE_NAME):
									...
									if (!empty($arr['has' . ucfirst($TYPE_NAME) . 'Scheme'])) {
										...
										$Table_name = $Table . '_' . $TYPE_NAME;
										$ARR_SCH    = $APP_SCH->findOne(['collection' => $table_name]);

										?>
										<div auto_tree>
											<div class="trait"><?= $Table_name ?></div>
										</div>
										<?
										// =>
										$arr_field   = $APP->plug('sitebase_app', 'appscheme_has_field')->distinct('idappscheme_field', ['idappscheme' => (int)$ARR_SCH['idappscheme']]);
											...
											<div class="autoBlock">
												<table class="table_form">
													<? while ($arrf = $rsF->getNext()) {
														...
														?>
														<tr>
															<td><?= ucfirst($arrf['field_title']) ?></td>
															<td>
																<div>
																	<?= chkSch($act_vars, $arr_test_has['idappscheme'], 'vars_has_table_field'); ?></div>
															</td>
