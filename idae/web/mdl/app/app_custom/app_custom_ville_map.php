...
<script>
	 ...
	 * DOM, with a file-local `cvm_` helper.
	 *
				...
				<tr>
					...
					<td>
						<button type="button"
						        class="validButton"
						        value="Situer"
						        onclick="searchMap(cvm_el('z<?= $uniqid ?>').value)">
...
<script type="text/javascript">
	...
	initializeMap = function () {
		...
		var map        = new google.maps.Map (cvm_el("map_canvas"), mapOptions);
		...
		google.maps.event.addListener (map, 'click', function (event) {
			...
			cvm_el('<?=$lat_field?><?=$uniqid?>').value = event.latLng.lat ();
			cvm_el('<?=$lng_field?><?=$uniqid?>').value = event.latLng.lng ();

			cvm_el('gps<?=$Table?>_lat').value = event.latLng.lat ();
			cvm_el('gps<?=$Table?>_lng').value = event.latLng.lng ();
			...
			geocoder.geocode (
				...
				function (results, status) {
					if ( status == google.maps.GeocoderStatus.OK ) {
						...
						placeMarker (loc)
						cvm_el('<?=$lat_field?><?=$uniqid?>').value = loc.lat ();
						cvm_el('<?=$lng_field?><?=$uniqid?>').value = loc.lng ();

						cvm_el('gps<?=$Table?>_lat').value = loc.lat ();
						cvm_el('gps<?=$Table?>_lng').value = loc.lng ();
