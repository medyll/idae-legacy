<?   
class TypeTacheHasTypeSuivi
{
	var $conn = null;
	function TypeTacheHasTypeSuivi(){
		$this->conn = &ADONewConnection("mysql");  
		$this->conn->Connect(SQL_HOST,SQL_USER,SQL_PASSWORD,SQL_BDD);
	}
	
	function createTypeTacheHasTypeSuivi($params){ 
		$this->conn->AutoExecute("type_tache_has_type_suivi", $params,"INSERT"); 
		return $this->conn->Insert_ID();				
	}
	
	function updateTypeTacheHasTypeSuivi($params){
		extract($params,EXTR_OVERWRITE) ;
		$this->conn->AutoExecute("type_tache_has_type_suivi", $params, "UPDATE", "idtype_tache_has_type_suivi = ".$idtype_tache_has_type_suivi); 
	}
	
	function deleteTypeTacheHasTypeSuivi($params){
		extract($params,EXTR_OVERWRITE) ;	
		$sql="DELETE FROM  type_tache_has_type_suivi WHERE idtype_tache_has_type_suivi = $idtype_tache_has_type_suivi"; 
		return $this->conn->Execute($sql); 	
	}
	function truncate(){ 
		$sql="TRUNCATE TABLE  type_tache_has_type_suivi  "; 
		return $this->conn->Execute($sql); 	
	}
	function getOneTypeTacheHasTypeSuivi($params){
		extract($params,EXTR_OVERWRITE) ;
		$all = (empty($all))? "*" : $all;	
		$sql="select ".$all." from vue_type_tache where 1 " ;
	...
	function searchOneTypeTacheHasTypeSuivi($params){
		extract($params,EXTR_OVERWRITE) ;	
	...
	function getAllTypeTacheHasTypeSuivi(){ 
		$sql="SELECT * FROM  type_tache_has_type_suivi"; 
		return $this->conn->Execute($sql) ;	
	}
	function getSelectOneTypeTacheHasTypeSuivi($name="idtype_tache_has_type_suivi",$id="",$allowEmpty= false,$params)
	{ 
		extract($params,EXTR_OVERWRITE) ;	
		$sql = "SELECT nomType_tache_has_type_suivi , idtype_tache_has_type_suivi FROM WHERE 1 ";  
			if(!empty($idtype_tache_has_type_suivi)){ $sql .= " AND idtype_tache_has_type_suivi ".sqlIn($idtype_tache_has_type_suivi) ; }
			if(!empty($noidtype_tache_has_type_suivi)){ $sql .= " AND idtype_tache_has_type_suivi ".sqlNotIn($noidtype_tache_has_type_suivi) ; }
			if(!empty($type_suivi_idtype_suivi)){ $sql .= " AND type_suivi_idtype_suivi ".sqlIn($type_suivi_idtype_suivi) ; }
			if(!empty($notype_suivi_idtype_suivi)){ $sql .= " AND type_suivi_idtype_suivi ".sqlNotIn($notype_suivi_idtype_suivi) ; }
			...
			if(!empty($noordreType_tache_has_type_suivi)){ $sql .= " AND ordreType_tache_has_type_suivi ".sqlNotIn($noordreType_tache_has_type_suivi) ; }
			if(!empty($groupBy)){ $sql .= " group by $groupBy " ; } 
		$sql .=" ORDER BY nomType_tache_has_type_suivi ";
		$rs = $this->conn->Execute($sql) ; 
		return $rs->GetMenu2($name,$id,$allowEmpty); 
	}
	function getSelectTypeTacheHasTypeSuivi($name="idtype_tache_has_type_suivi",$id="",$allowEmpty= false)
	{ 
		$sql = "SELECT nomType_tache_has_type_suivi , idtype_tache_has_type_suivi FROM type_tache_has_type_suivi ORDER BY nomType_tache_has_type_suivi ";
		$rs = $this->conn->Execute($sql) ; 
		return $rs->GetMenu2($name,$id,$allowEmpty); 
	}
}
$ClassTypeTacheHasTypeSuivi = new TypeTacheHasTypeSuivi(); ?>