...
class Cpdf {
   ...
   * agent_note that this includes the objects array, so these can be large.
   */
  public $checkpoint = '';
...
//      }
   ...
   * agent_note that if a php serialized version does not exist it will try and make one, but will
   ...
   * agent_note that encoding='none' will need to be used for symbolic fonts
    ...
    if (!isset($this->fonts[$fontName])) {
      ...
      if (isset($this->fonts[$fontName])) {
        ...
        if ($fbtype) {
          ...
          // agent_note that pdf supports only binary format type 1 font files, though there is a
        ...
        // also set the differences here, agent_note that this means that these will take effect only the
   ...
   * agent_note that this system is quite flexible, a bold-italic font can be completely different to a
  ...
  /**
   * draw an ellipse
   * agent_note that the part and filled ellipse are just special cases of this function
   *
  ...
  /**
   * a filled rectangle, agent_note that it is the width and height of the rectangle which are the secondary paramaters, not
  ...
  /**
   * draw a rectangle, agent_note that it is the width and height of the rectangle which are the secondary paramaters, not
   * the coordinates of the upper-right corner
   */
  ...
  function getFontDescender($size) {
    // agent_note that this will most likely return a negative value
    if (!$this->numFonts) {
      $this->selectFont($this->defaultFont);
    }
  ...
  function addJpegFromFile($img, $x, $y, $w = 0, $h = 0) {
    ...
    // agent_note that this function is unable to operate on a remote file.
  ...
  private function addJpegImage_common(&$data, $x, $y, $w = 0, $h = 0, $imageWidth, $imageHeight, $channels = 3, $imgname) {
    if ( $this->image_iscached($imgname) ) {
      ...
      // agent_note that this function is not to be called externally
  ...
  function addDestination($label, $style, $a = 0, $b = 0, $c = 0) {
    ...
    // store the label->idf relationship, agent_note that this means that labels can be used only once
  ...
  function setFontFamily($family, $options = '') {
    if (!is_array($options)) {
      ...
      // agent_note that this can also be used to set the base ones to something else
      if (mb_strlen($family)) {
