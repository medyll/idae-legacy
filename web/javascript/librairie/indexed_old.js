	...
	function indexed(dbstore) {
		...
		return {
			...
			parseQuery: function (query) {
				...
				for (var property in query) { // MYDDE
				 if (query.hasOwnProperty(property)) {
				 var tm_qry = []
				 tm_qry[property] = query[property];
				 query_arr.push(tm_qry);
				 }
				 }
				query_arr.forEach(function (cond) {
					// Set key
			...
			create: function (cb) {
				...
				request.onupgradeneeded = function (e) {
					...
					db.createIndex('file_var', ['file','vars'], {unique:false});
					db.createIndex('key_cache', 'key_cache', {unique:false});
				};
