...
var ssl_opt = {
	key: fs.readFileSync('/etc/letsencrypt/live/idaertys-preprod.mydde.fr-0002/privkey.pem'),
	cert: fs.readFileSync('/etc/letsencrypt/live/idaertys-preprod.mydde.fr-0002/fullchain.pem'),
	ca: fs.readFileSync('/etc/letsencrypt/live/idaertys-preprod.mydde.fr-0002/chain.pem'),
	requestCert: false,
...
var app = require('https').createServer(ssl_opt,http_handler)
...
function http_handler(req, res) {
	...
	console.log('http_handler',req.url);
...
function socket_onconnection() {
	  ...
	  console.log('socket_onconnection');
		...
		socket.on('disconnect', function (data) {
			...
			if (socket.idagent) {
				mongoClient.connect(mongo_url, {server: {auto_reconnect: true, disconnect: true}}, function (error, db) {
					...
					table_agent.update({idagent: eval(data.IDAGENT)}, {'$set': {onlineAgent: 0}}, {upsert: true})
					 console.log('disconnect https', socket.idagent)
				});
			}
...
module.exports = {
	...
	socket_start: function (port) {
		//
		console.log('app_http / socket_start on port ' + port)
