...
var app                 = require('http').createServer(handler),
    io                  = require('socket.io').listen(app),_mysql
    fs                  = require('fs'),
    qs                  = require('qs'),
    mysql               = require('mysql'),
...
// sys_auditlogtable => tout est logué ici ?  

var tables = ['t_servclt','t_solservclt','t_solservpredef', 't_inter','t_interinfoprincipales','t_motifdmdeinter','t_moduleservclt','t_affaire','t_financement','t_site','t_art','t_bien','t_partie','t_cptcpta','t_adrmedia','t_volumemoyenmensuel','t_prix'];
var arr_date = [];
var index ;
...
var query_loop = function(table){
	...
	var query = connection.query("SELECT *, DATE_FORMAT(T_UPDATE_DATE,'%d %b %Y %T:%f') as DADATE  FROM  " +table+ " order by T_UPDATE_DATE desc, T_COMPUTED_UPDATE_DATE desc limit 200", function(err, rows) {		 
		if(!rows) return;		
		if(rows.length==0) return;		
		result_date = rows[0].DADATE;  
		 	if( arr_date[table] != result_date ){
		 		console.log('changes ' , table);
		 		...
		 		console.log(table,' <=   ', rows[0].DADATE); 
				/*for (var i in rows) {
				    	console.log(table,' DADATE : ', rows[i].DADATE); 
				  		arr_date[table] = result_date;				  		
				  		vars['rows']  = rows;
				  		vars['table'] = table;
				  		//
...
var pollingSysLoop = function () { 
   ...
   var query = connection.query("SELECT *, DATE_FORMAT(T_DATE_EVENT,'%d %b %Y %T:%f') as DADATE  FROM sys_auditlogtable order by T_DATE_EVENT desc limit 1");
